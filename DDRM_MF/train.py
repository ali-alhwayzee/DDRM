											...
											for dims in dims_list:
												for i, scale in enumerate(scale_list):
													...
													if check_ckpt(lr, dlr, layer, alpha, steps, int(steps*sample), min_, max_, dims, scale, act, beta, log, log_num):
														...
														pid = load_program(lr, dlr, layer, alpha, steps, int(steps*sample), min_, max_, dims, scale, act, beta, log, log_num, gpu=gpu_list[log_num%1])
													else:
														pid = run_program(lr, dlr, layer, alpha, steps, int(steps*sample), min_, max_, dims, scale, act, beta, log, log_num, gpu=gpu_list[log_num%1])
